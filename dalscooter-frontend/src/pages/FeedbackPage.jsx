  ...
  const loadAvailableBikes = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/bikes`,
        {
          headers: {
              ...
              <div className="modal-body">
                ...
                {bikes.length > 0 && (
                  <div className="form-group">
                    <label className="form-label">Select Bike (Optional)</label>
                    <select
        ...
        {!user && (
          <div className="guest-message">
            <div className="guest-message-content">
              ...
              <div>
                <h3 className="guest-message-title">
                  Want to share your experience?
                </h3>
                <p className="guest-message-text">
                  Sign in to submit feedback and help other customers make
                  informed decisions.
                </p>
                <a href="/login" className="guest-message-button">
...
const MyFeedback = ({ user }) => {
  ...
  const loadMyFeedback = async () => {
    try {
      ...
      const result = await feedbackService.getCustomerFeedback(
        customerId,
        user.idToken
      );
            ...
            <div key={feedback.feedback_id} className="feedback-item">
              <div className="feedback-item-header">
                <div className="feedback-item-info">
                  <span className="feedback-bike-id">
                    Bike: {feedback.bike_id}
                  </span>
                ...
                <span
                  className={`feedback-sentiment ${feedback.sentiment.toLowerCase()}`}
                >
                  {feedback.sentiment}
                </span>
              </div>
              ...
              <div className="feedback-meta">
                ...
                <span>
                  Confidence:{" "}
                  {(
                    (feedback.confidence || feedback.sentiment_score) * 100
                  ).toFixed(1)}
                  %
                </span>
              </div>
