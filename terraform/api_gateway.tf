...
resource "aws_lambda_permission" "api_post_chat" {
  statement_id  = "AllowAPIGWInvokePostChat"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.chat_post.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_api_gateway_rest_api.api.execution_arn}/*/POST/concerns/*/chats"
}
resource "aws_lambda_permission" "api_get_chat" {
  statement_id  = "AllowAPIGWInvokeGetChat"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.chat_get.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_api_gateway_rest_api.api.execution_arn}/*/GET/concerns/*/chats"
}

# API Gateway: add /concerns/{concernId}/chats
resource "aws_api_gateway_resource" "concern_chats" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  parent_id   = aws_api_gateway_resource.bookings.id # or root: api.root_resource_id
  path_part   = "concerns"
}
resource "aws_api_gateway_resource" "concern_id" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  parent_id   = aws_api_gateway_resource.concern_chats.id
  path_part   = "{concernId}"
}
resource "aws_api_gateway_resource" "concern_chats_sub" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  parent_id   = aws_api_gateway_resource.concern_id.id
  path_part   = "chats"
}

# POST /concerns/{concernId}/chats
resource "aws_api_gateway_method" "post_concern_chat" {
  rest_api_id   = aws_api_gateway_rest_api.api.id
  resource_id   = aws_api_gateway_resource.concern_chats_sub.id
  http_method   = "POST"
  authorization = "COGNITO_USER_POOLS"
  authorizer_id = aws_api_gateway_authorizer.cognito_authorizer.id
}
resource "aws_api_gateway_integration" "post_concern_chat" {
  rest_api_id             = aws_api_gateway_rest_api.api.id
  resource_id             = aws_api_gateway_resource.concern_chats_sub.id
  http_method             = aws_api_gateway_method.post_concern_chat.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.chat_post.invoke_arn
}

# GET /concerns/{concernId}/chats
resource "aws_api_gateway_method" "get_concern_chat" {
  rest_api_id   = aws_api_gateway_rest_api.api.id
  resource_id   = aws_api_gateway_resource.concern_chats_sub.id
  http_method   = "GET"
  authorization = "COGNITO_USER_POOLS"
...
resource "aws_api_gateway_integration" "get_concern_chat" {
  rest_api_id             = aws_api_gateway_rest_api.api.id
  resource_id             = aws_api_gateway_resource.concern_chats_sub.id
  http_method             = aws_api_gateway_method.get_concern_chat.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.chat_get.invoke_arn
}

# CORS for /concerns/{concernId}/chats
resource "aws_api_gateway_method" "concern_chats_options" {
  rest_api_id   = aws_api_gateway_rest_api.api.id
  resource_id   = aws_api_gateway_resource.concern_chats_sub.id
  http_method   = "OPTIONS"
  authorization = "NONE"
}
resource "aws_api_gateway_integration" "concern_chats_options" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.concern_chats_sub.id
  http_method = aws_api_gateway_method.concern_chats_options.http_method
  type        = "MOCK"
  request_templates = { "application/json" = jsonencode({ statusCode = 200 }) }
}
resource "aws_api_gateway_method_response" "concern_chats_options" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.concern_chats_sub.id
  http_method = aws_api_gateway_method.concern_chats_options.http_method
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "concern_chats_options" {
  depends_on = [aws_api_gateway_integration.concern_chats_options]
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.concern_chats_sub.id
  http_method = aws_api_gateway_method.concern_chats_options.http_method
  status_code = aws_api_gateway_method_response.concern_chats_options.status_code
  response_parameters = {
    "method.response.header.Access-Control-Allow-Headers" = "'Content-Type,X-Amz-Date,Authorization,X-Api-Key,X-Amz-Security-Token'"
    "method.response.header.Access-Control-Allow-Methods" = "'GET,POST,OPTIONS'"
    "method.response.header.Access-Control-Allow-Origin"  = "'*'"
