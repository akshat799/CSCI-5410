...
resource "aws_api_gateway_resource" "concern_id" {
  rest_api_id = aws_api_gateway_rest_api.bike_api.id
  parent_id   = aws_api_gateway_resource.concerns.id
  path_part   = "{concernId}"
}
...
resource "aws_api_gateway_integration" "post_chat" {
  ...
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.lambda["chat_post"].invoke_arn
}

resource "aws_api_gateway_method_response" "post_chat" {
  rest_api_id = aws_api_gateway_rest_api.bike_api.id
  resource_id = aws_api_gateway_resource.chats.id
  http_method = aws_api_gateway_method.post_chat.http_method
  status_code = "201"
...
resource "aws_api_gateway_integration" "get_chats" {
  ...
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.lambda["chat_get"].invoke_arn
}

resource "aws_api_gateway_method_response" "get_chats" {
  rest_api_id = aws_api_gateway_rest_api.bike_api.id
  resource_id = aws_api_gateway_resource.chats.id
  http_method = aws_api_gateway_method.get_chats.http_method
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "chats_options" {
  ...
  resource_id = aws_api_gateway_resource.chats.id
  http_method = aws_api_gateway_method.chats_options.http_method
  status_code = aws_api_gateway_method_response.chats_options.status_code
...
# Lambda Permissions
resource "aws_lambda_permission" "api_gateway_chat_post" {
  statement_id  = "AllowExecutionFromAPIGatewayChatPost"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.lambda["chat_post"].function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_api_gateway_rest_api.bike_api.execution_arn}/*/*"
}

resource "aws_lambda_permission" "api_gateway_chat_get" {
  statement_id  = "AllowExecutionFromAPIGatewayChatGet"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.lambda["chat_get"].function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_api_gateway_rest_api.bike_api.execution_arn}/*/*"
}